...
class Block {
  ...
  public function __construct($block) {
    $this->break = $this->block_break($block); // escaping the container, for fullwidth blocks (not needed in previews)
    $this->container = $this->block_container($block); // inner container settings; for 'left|center|right' block alignment as well as 'wide|full' width modes (not needed in previews)
    $this->background = $this->block_background($block); // background image settings
    $this->block_wrapper_attributes = $this->block_wrapper_attributes($block); // inbuilt block attributes; custom classname is removed from here and placed into markup via card instead (background settings like colour get added to this wrapper in previews, so use the preview wrap here instead)
